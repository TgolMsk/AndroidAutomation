...
- **设备与账号**：模拟器实例页（启动 / 停止 / 重启 / 实时画面 / 新建 / 克隆 / 删除，停止、重启、删除前检查占用；运行中的实例一键「重启游戏」——强制停止后重新拉起，用于 AI 处理不了的卡界面、弹窗、断线）；基础实例与批量克隆（1–8 个，默认轮换设备标识，继承模板集）；账号按实例创建时间绑定，交互式手机号登录，登录后按城内 / 世界地图模板验证。
- **自动采集**：G0–G16 采集状态机，按部队面板倒计时做 ETA 调度（到点先只读采样，有空位才派兵）。冷启动时用 monkey 拉起游戏，门槛前只允许关弹窗、一次 BACK 加「取消」退出框。首次启用前要做只读探针并确认。
- **运行记录**：脚本控制台（勾选多台模拟器一次执行同一个脚本，带各自绑定账号的参数，一起暂停 / 继续 / 停止）；脚本计划（按账号勾选脚本，北京时间每天 / 间隔触发）。脚本最高优先：执行期间这台实例上的其他自动功能全部让路（自动采集、健康探针与卡死看门狗、读资源统计、机器人操作、手动采集），临时运行默认当场抢占，结束 15 秒后恢复。执行监控页提供实时日志、留痕截图、暂停 / 继续 / 停止。
...
- **脚本与模板**：JSON 脚本 DSL 与可视化块编辑器（从画面截取模板直接成块，模板选择器按界面分组、默认不列数字字形）；模板库按分类筛选、搜索、缩略图分页，支持去底预览、差分掩码与方差守卫，「导入 / 合并旧模板集」只增不改。
- **设置**：通知与推送（本机 / Telegram，凭据用系统钥匙串加密）、异常告警（掉线 / 顶号 / 连续失败时先暂停再推送；被顶号时默认再关闭该模拟器）、卡死看门狗（「卡死自动重启」默认关闭）、Telegram 机器人（只认 Chat ID 加授权用户 ID；「查看状态与截图」与「远程操作」两个开关，默认都关）、留痕策略、应用内更新（查 GitHub Release、校验 SHA256 后打开 DMG）。
