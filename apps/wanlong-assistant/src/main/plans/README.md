...
8. **AI 介入**：计划设置 `aiAssist`（默认开，原版）随每次运行交给执行器（关掉时步骤失败不再问 AI）；是否真的动手由 AI 模块决定
   （AI 顾问启用且允许自动处理）。AI 模块也可调用 `PlanService.aiAssistEnabled(gameId)`。
...
`views/automation/PlanPanel.tsx` 的 `plans` 模式已被新的「任务计划」页取代，只剩脚本页在用它的 `scripts` 模式。
