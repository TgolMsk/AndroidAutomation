...
6. **留痕策略** `never / onFail / always`：默认取宿主端口 `shotPolicy()`（应用设置；`index.ts` 在应用设置服务合入后接上），
   启动执行弹窗默认「跟随应用设置」，只有用户改过才覆盖。
7. **整体时限**：引擎自己的 `maxRunMs` 计时（含暂停时间）到点即失败；主进程另有兜底（`maxRunMs` + 30 秒），
   线程卡住 / 设备调用不可打断时先请求停止、10 秒后终止线程，结果记为「超过时间上限」的失败。
