...
6. **留痕策略** `never / onFail / always`：默认取宿主端口 `shotPolicy()`，`src/main/index.ts` 接的是应用设置服务
   ...
   **匹配默认值**：宿主端口 `matchDefaults()`（应用设置的 `matchThreshold` / `shrink`）随每次执行交给线程：没写阈值的模板用
   ...
   设备调用经 `deviceHost`（DeviceLane），与采集、探针、机器人截图共用每实例的串行通道与最小截图间隔；线程里「这一帧还能复用」
   的窗口也取应用设置的「单实例最小截图间隔」（`ScriptRunnerOptions.captureIntervalMs`，原版 worker/context.ts 同样取它）。
