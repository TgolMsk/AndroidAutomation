    ...
    try {
      ...
      if (!run.controller.signal.aborted) {
        ...
        this.emitError(state, this.entries.get(run.key)?.enabled === false
          ? new Error(error instanceof SchedulePauseError
            ? `安全保护已暂停自动调度：${error.message}`
            : `连续 ${this.maxConsecutiveFailures} 次失败，自动调度已暂停：${error instanceof Error ? error.message : String(error)}`)
          : error);
      }
    } finally {
