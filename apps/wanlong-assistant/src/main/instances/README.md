...
- `provisioner.ts`：`InstanceProvisioner`
  - `view(gameId)` 按实时实例校验：基础实例被删除或编号被新实例复用（创建时间不同）时自动取消。只有真正清掉它的那一次调用
    在返回值与事件里带 `cleared`（含 `setAt`，界面据此去重）；并发检查或期间已设了新基础实例时返回当前状态、不推送。
  ...
  - `cloneFromBase(gameId, { count 1–8, expectedBaseIndex, rotateIdentity })`：基础实例必须存在、身份一致且已关机；
    ...
    （同一个源一次复制、失败整体回滚）；默认给每个副本新的设备标识（`identity: 'random'`），取消勾选时显式传
    `identity: 'system'`（core 对带托管标识的源默认会换，不传等于没关）；副本继承基础实例的模板集与采集设置。
