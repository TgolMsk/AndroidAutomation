...
async function settle(key: string, label: string, timeoutMs: number, hint: string, probe: () => Promise<HealthItem>): Promise<HealthItem> {
  try {
    ...
    return item(key, label, 'fail', /[\u4e00-\u9fa5]/.test(text) ? text : `检查失败：${text}`, hint);
  }
}
...
export async function runAssistantHealthCheck(deps: HealthDeps): Promise<HealthReport> {
  ...
  items.push(await settle('templates', '模板集', timeout, '到「脚本与模板 → 模板库」为该实例选择或导入模板集', async () => {
    const targets = await deps.templateTargets();
    if (targets.length === 0) return item('templates', '模板集', 'ok', '没有启用自动采集的实例，跳过检查');
    const good: string[] = [];
    const bad: string[] = [];
    for (const target of targets) {
      try {
        const set = await withTimeout(target.load(), timeout, `实例 #${target.index} 的模板集读取`);
        if (!set) bad.push(`实例 #${target.index} 还没有选择模板集`);
        else if (set.templates === 0) bad.push(`实例 #${target.index} 的模板集「${set.name}」是空的`);
        else good.push(`实例 #${target.index}「${set.name}」${set.templates} 张`);
      } catch (error) {
        bad.push(`实例 #${target.index} 的模板集无法读取：${message(error)}`);
      }
    }
    if (bad.length > 0) {
