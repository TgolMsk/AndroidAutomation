...
`@avdm/core` 是模拟器操作的唯一入口。实例注册表、运行记录、启动端口与设备发现均由它维护，并使用已有文件锁保护跨进程修改。助手保存的账号绑定、任务配置、模板、统计、通知和顾问数据放在 `~/.avdm/automation/` 下，按游戏和实例隔离；凭据与真实截图不进入源码或安装包。万龙觉醒的内置模板库（`apps/wanlong-assistant/resources/templates/`）随助手安装包分发，启动时只增不改地补进 `~/.avdm/automation/templates/wanlong/`，之后的编辑、自学都只改用户那份。删除或重建同编号实例时，助手应依据实例创建标识检查旧绑定，不能仅按 `index` 自动继承账号。可复用方法、参数和命令示例见 [模拟器核心 API 与 CLI](EMULATOR_API.md)。
