...
- `prepareFrame` / `prepareTemplate` / `matchTemplate` (`matchIn`) / `detect`: TM_CCOEFF_NORMED only; std < 12 is rejected (`TEMPLATE_LOW_VARIANCE`); alpha PNGs become masks (cubic resize, ≥128 opaque; fully transparent or < 64 px / < 10 % rejected); a ROI outside the frame or smaller than the template is a miss with a Chinese reason, never an exception. Errors are `AppError`s with a `code` (`src/errors.ts`, re-exported by `wanlong/errors.ts`). As in the original panel, `AppError.from(e, fallback)` keeps only the code of an `AppError` or a serialized one (the `__wlError` marker); Node / core errors get the fallback.
...
- `TemplateLibrary.save(dir, draft)`: optional crop, `alpha` or server-side `diffFrames` / `diffTolerance`, fixed `id` with explicit `overwrite`, stable `<id>.png`, metadata (`std`, `maskCoverage`, `note`, `tags`, `createdAt`, `updatedAt`). `importSets(gameId, sourceDir, { dryRun })` = `mergeTemplateSets` (only-add legacy import; `found` = sets in the source, `dryRun` reports without writing). Set folders, write locks and change events use one canonical spelling (`canonicalDirectory`, `canonicalGameRoot`).
