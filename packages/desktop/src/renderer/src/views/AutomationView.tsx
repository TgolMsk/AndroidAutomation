...
type BusyAction = 'template' | 'save' | 'probe' | 'run' | 'stop' | null;
...
export function AutomationView({ onBack }: { onBack: () => void }) {
  ...
  async function chooseTemplate(): Promise<void> {
    if (!game || index === null || busy) return;
    setBusy('template');
    try {
      const directory = await avdm.pickAutomationTemplateSet();
      if (!directory) return;
      const saved = await avdm.saveAutomationSettings(game.id, index, { templateDir: directory });
      setSettings(saved);
      setProbe(null);
      setProbeError(undefined);
      setProbeConfirmed(false);
      toast.push({ kind: 'success', title: '模板目录已保存' });
    } catch (error) {
      toast.error('无法保存模板目录', errMsg(error));
    } finally {
      setBusy(null);
    }
  }
  ...
  return (
    <div className="automation-view">
      <header className="automation-header">
        <button className="btn ghost automation-back" onClick={onBack}><Icon name="back" />返回实例</button>
        <div className="automation-heading">
          <span className="automation-heading-icon"><Icon name="workflow" size={19} /></span>
          <div><h1>自动化</h1><p>选择游戏与实例，校准画面后启动任务。</p></div>
        </div>
      ...
      <div className="automation-shell">
        <aside className="automation-sidebar" aria-label="游戏包">
          <div className="automation-side-heading"><span>游戏包</span><span>{games.length}</span></div>
          {gamesError && <div className="automation-side-error" role="alert">{gamesError}<button className="btn xs" onClick={() => void loadGames()}>重试加载</button></div>}
          {!gamesLoaded && <div className="automation-skeleton" aria-label="正在加载游戏包" />}
          {gamesLoaded && games.length === 0 && !gamesError && <p className="automation-side-empty">尚无已安装的游戏包。游戏逻辑将以独立模块接入。</p>}
          <div className="automation-game-list">
            {games.map((item) => (
              <button key={item.id} className={`automation-game ${item.id === gameId ? 'is-active' : ''}`} onClick={() => setGameId(item.id)} aria-current={item.id === gameId ? 'page' : undefined}>
                <span className="automation-game-mark"><Icon name="package" size={17} /></span>
                <span className="automation-game-text"><strong>{item.name}</strong><small>{item.packageName}</small></span>
                <span className="automation-game-count">{item.tasks.length}</span>
              </button>
            ))}
          </div>
        </aside>
        ...
        <main className="automation-main">
          {!game ? (
            <div className="automation-empty"><Icon name="package" size={28} /><h2>选择游戏包</h2><p>游戏模块负责各自的任务和配置，模拟器实例仍由 AVD 管理器统一控制。</p></div>
          ) : (
            <div className="automation-content">
              ...
              <section className="automation-section" aria-labelledby="automation-target-title">
                <div className="automation-section-title"><h3 id="automation-target-title">目标与模板</h3><p>配置按游戏和实例分别保存。</p></div>
                ...
                <div className="automation-template-row">
                  <div className="automation-template-copy"><span>本地模板目录</span><strong className={settings?.templateDir ? 'mono' : ''}>{settings?.templateDir || '尚未选择'}</strong></div>
                  <button className="btn" onClick={() => void chooseTemplate()} disabled={index === null || settingsLoading || Boolean(busy)}>
                    {busy === 'template' ? <Spinner size={14} /> : <Icon name="folder" />}选择目录
                  </button>
                </div>
                {!settings?.templateDir && <div className="automation-guidance"><Icon name="info" size={16} /><span>选择包含 <code>manifest.json</code> 和 PNG 图片的模板目录。模板只从本机读取；完成选择后才能探测。</span></div>}
              </section>
