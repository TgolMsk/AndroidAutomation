...
/** Create new instances from an installed system image (shared by the emulator manager and product apps). */
export function CreateDialog({
  ...
  /** Fields that override the settings' default spec as the form's starting values (e.g. a product's resolution). */
  initialSpec?: Partial<InstanceSpec>;
  /** Extra note shown above the form. */
  notice?: ReactNode;
}) {
  ...
  const [spec, setSpec] = useState<InstanceSpec>(() => ({ ...settings.defaultSpec, ...initialSpec, extraArgs: [...(initialSpec?.extraArgs ?? settings.defaultSpec.extraArgs)] }));
