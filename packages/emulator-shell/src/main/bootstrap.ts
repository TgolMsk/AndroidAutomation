...
import { registerIpcHandlers, type MainServices } from './ipc-handlers';
...
export interface AppEdition {
  name: string;
  /** HTML page produced by this product's renderer build. */
  rendererPage?: string;
  createAddon?: (services: MainServices, home: string) => {
    restore?(): Promise<void>;
    dispose?(): Promise<void>;
  };
}

/** Reusable Electron shell. Product extensions register their own IPC and services. */
export function bootstrapApp(edition: AppEdition): void {
  app.setName(edition.name);
  ...
  const windows = new WindowManager(host, live, edition.name, edition.rendererPage ?? 'index.html');
  const sdkInstall = new SdkInstallTask();
  const services: MainServices = { host, live, thumbs, windows, sdkInstall };
  registerIpcHandlers(services);
  const addon = edition.createAddon?.(services, defaultHome());
  ...
  void app.whenReady().then(() => {
    ...
    void addon?.restore?.().catch((error: unknown) =>
      console.error('[avdm] 扩展服务恢复失败:', error));
  ...
  const shutdown = async () => {
    ...
    if (addon?.dispose) await withTimeout(addon.dispose(), 15_000, undefined);
    thumbs.dispose();
    live.dispose();
  ...
  win.webContents.once('did-finish-load', () => {
    void (async () => {
      try {
        ...
        const connected = await subject.webContents.executeJavaScript('typeof window.avdm?.listInstances === "function"');
        if (!connected) throw new Error('预加载脚本未连接到主进程');
