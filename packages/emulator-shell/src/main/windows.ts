...
function loadRoute(win: BrowserWindow, route: string, page: string): Promise<void> {
  const dev = devServerUrl();
  if (dev) return win.loadURL(`${dev.replace(/\/$/, '')}/${page}#${route}`);
  return win.loadFile(join(here, '../renderer', page), { hash: route });
}
...
export class WindowManager {
  ...
  constructor(
    ...
    private readonly appTitle = 'AVD 多开管理器',
    private readonly rendererPage = 'index.html',
  ) {
  ...
  showMain(): BrowserWindow {
    ...
    void loadRoute(win, '/', this.rendererPage);
    this.main = win;
    return win;
  }
  ...
  private async createLive(index: number): Promise<void> {
    ...
    await loadRoute(win, `/live/${index}`, this.rendererPage);
  }
